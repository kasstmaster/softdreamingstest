...
prize_storage_message_id: int | None = None
...
tb = "".join(traceback.format_exception(type(exc), exc, exc.traceback))
text = f"{tag}: {exc}\n{tb}"
text = "@everyone " + text
if len(text) > 1900:
text = text[:1900]
...
cfg["infected_members"] = json.dumps({str(k): v for k, v in infected_guild.items()})
cfg["plague_scheduled"] = json.dumps(plague_scheduled_guild)
...
def init(self, gift_title: str, rarity: str):
super().init()
